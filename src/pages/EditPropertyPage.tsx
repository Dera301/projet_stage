...
const uploadNewImages = async (): Promise<string[]> => {
  ...
  try {
    const uploadImageToServer = async (file: File): Promise<string> => {
      ...
      // Gérer la réponse qui peut contenir du HTML
      const responseText = await response.text();
      
      // Nettoyer la réponse des balises HTML
      const cleanText = responseText
        .replace(/<br\s*\/?>/gi, '') // Supprimer les <br>
        .replace(/<b>.*?<\/b>/gi, '') // Supprimer les balises <b>
      ...
      try {
        ...
        console.error('Texte original:', responseText);
      ...
      // Retourner l'URL de l'image selon la structure de votre API
      return data.imageUrl || data.data?.imageUrl || data.url;
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await fetch(`http://localhost/Projet_stage/api/properties/update.php?id=${property.id}`, {
        method: 'POST', // InfinityFree bloque PUT, donc on reste sur POST
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(updateData),
    });

    // Vérifie si le serveur renvoie bien du JSON
    const text = await response.text();
    console.log("Réponse brute du serveur (text):", text);

    let data;
    try {
        data = JSON.parse(text);
    } catch (parseError) {
        console.error("❌ Erreur de parsing JSON — voici la réponse complète du serveur :");
        console.error(text); // Affiche la page HTML complète de l'erreur PHP
        throw new Error("Le serveur n’a pas renvoyé du JSON valide. Vérifie ton script PHP.");
    }
